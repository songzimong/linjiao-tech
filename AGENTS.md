...
> 每项任务分发前必须经过难度评估，杜绝简单任务烧重型资源。

### 难度判定矩阵

| 难度级 | 判定条件 | 示例 | 路由目标 |
|--------|----------|------|----------|
| **L1 轻量** | 只读查询/文件列表/简单搜索 | "找一下上周的会议纪要"、"列出D盘PDF" | 轻量虾兵（白泽/谛听/毕方） |
| **L2 标准** | 内容生成/格式转换/中等分析 | "把这份PDF转成Word"、"写一篇周报" | 标准虾兵 |
| **L3 重型** | 深度推理/多步决策/跨域编排 | "设计一个新架构方案"、"分析竞品并出报告" | 重型虾兵（青龙/朱雀/玄武/饕餮） |

### 路由决策流程

```
输入任务 → 难度评估 → 
  ├─ L1 → 直接分配给轻量虾兵，设 maxTurnCount=5
  ├─ L2 → 分配给标准虾兵，设 maxTurnCount=10
  └─ L3 → 分配给重型虾兵，设 maxTurnCount=15，必要时启动蜂群模式
```
...
| 熔断层 | 触发条件 | 动作 | 配置位置 |
|--------|----------|------|----------|
| **虾兵自检** | 单虾兵对话超过 10 轮 | 自动终止当前任务，返回部分结果 | `subagents.maxTurnCount: 10` |
| **麒麟监察** | 虾兵 spawn 的子任务超过 4 分钟 | 强制回收，记录超时日志 | `subagents.taskTimeout: 240000` |
| **全局熔断** | 单个主任务总消耗超过 50 轮对话 | 中断所有子任务，向用户汇报 | 麒麟运行时检测 |

### 死锁检测规则

满足以下任意条件时判定为死锁，立即中断：

1. 虾兵连续 3 轮返回相同内容（语义重复检测）
2. 虾兵输出中出现 `"我需要更多数据"` + `"请明确你需要什么数据"` 循环模式
3. 任务 5 分钟内没有任何子任务产生实质性产出（文件写入/信息返回）

### 熔断后处理

```
中断 → 已产出部分归档 → 写入 memory/bus/circuit-break-{timestamp}.md → 向用户报告
```
...
| 权限级 | 标识 | 虾兵 | 允许工具 | 禁止操作 |
|--------|------|------|----------|----------|
| **🟢 只读** | READ_ONLY | 白泽/重明/谛听/毕方/商羊/金乌 | read_text/web_search/web_fetch/analyze_image/sessions_list | 任何文件写入/删除/shell执行 |
| **🟡 内容** | CONTENT | 青龙/朱雀/应龙/饕餮/貔貅/灵芝/鹊桥/离珠/太史/净坛/辟邪/龟甲 | 读+写文件+搜索+图片分析 | shell_executor(禁止)、系统配置修改 |
| **🔴 运维** | OPS | 玄武/穷奇/刑天/鲲鹏 | 全部工具 | 格式化/系统关键路径删除需麒麟二次确认 |

### 权限执行方式

- 由麒麟在 `sessions_spawn` 的 task 描述中注入权限约束
- 格式：`[权限级: READ_ONLY] [熔断: maxTurnCount=5] 你的任务描述`
- 虾兵收到权限标识后，必须严格遵守，违规操作麒麟有权收回

---

## 原始调度原则（继续）

11. 每次派发前执行难度评估，按路由矩阵分发
12. 每次派发时在 task 前追加权限标识和熔断参数
13. 监控所有子任务，超过阈值触发熔断
14. 熔断后必须记录日志并通知用户
